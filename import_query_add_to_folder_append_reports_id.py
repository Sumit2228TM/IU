...
def import_query(filepath):
    ...
    original_id = filename.replace("query_", "").replace(".json", "")
    ...
    data["title"] = f"{original_title}-from_reports.{original_id}"
    ...
    print(f"  [OK] {filename} imported as '{data['title']}' -> new query ID {new_id}")
    return new_id
